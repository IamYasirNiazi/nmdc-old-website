...
// $t_aggregate_order = "DESC";
if(isset($_POST['t_aggregate_order']) && $_POST['t_aggregate_order'] == "ASC"){
    $t_aggregate_order = "ASC";
}else{
    $t_aggregate_order = "DESC";
}
$finalQuery = "SELECT ROUND(((`students_data`.`matric_marks`/`students_data`.`matric_tmarks`)*10) + 
((`students_data`.`fsc_marks`/`students_data`.`fsc_tmarks`)*40) + ((`students_data`.`mcat_marks`/`students_data`.`mcat_tmarks`)*50),2) as t_aggregate,`students_data`.*,`students_application`.`applied_for` as `course_applied` FROM `students_data` JOIN `students_application` ON students_data.std_id = students_application.std_id $queryCondition ORDER BY t_aggregate $t_aggregate_order";
...
if(isset($_POST['exportData'])){
    ...
    // Column names 
    // $fields = array('Date', 'Student Name', 'Father Name', 'Gender', 'Marital Status', 'DOB', 'Domicile', 'Place of Birth', 'Status of Applicant', 'CNIC', 'Passport', 'Applicant Mobile', 'Email Address', 'Current Residential Address', 'Father Contact', 'Land Line', 'Matriculation/O Level', 'Year of Passing', 'Institution Name', 'Roll no.', 'Marks Obtained', 'Total Marks', 'Percentage', 'F.Sc/A Level', 'Year of Passing', 'Institution Name', 'Roll no.', 'Marks Obtained', 'Total Marks', 'Percentage', 'MCAT', 'Year of Passing', 'Institution Name', 'Roll no.', 'Marks Obtained', 'Total Marks', 'Percentage');
    $fields = array('Student Name', 'Father Name','CNIC', 'Gender', 'DOB', 'Address', 'Contact No.', 'Email',  'SSC Passing Year', 'SSC Total Marks', 'SSC Marks Obtained', 'SSC Obtained Weightage (10%)', 'FSC/A-level Passing year','HSSC/IBCC Equilvalence Total Marks', 'HSSC/IBCC Equilvalence Marks Obtained',  'FSC Obtained Weightage (40%)', 'MDCAT Year','MDCAT Roll No.', 'MDCAT Total Marks', 'MDCAT Marks Obtained', 'MDCAT Obtained Weightage (50%)', 'Total Aggregate');
    ...
    if ($query->num_rows > 0) {
        ...
        while ($row = $query->fetch_assoc()) {
            ...
            // $lineData = array($row['date'], $row['name'], $row['fname'], $row['gender'], $row['mstatus'], $row['dob'], $row['domicile'], $row['pbirth'], $row['applicant_status'], $row['cnic'], $row['passport'], $row['app_mob'], $row['email'], $row['caddress'], $row['fathcontact'], $row['landline'], $row['matric'], $matric_year, $row['matric_inst'], $row['matric_roll'], $row['matric_marks'], $row['matric_tmarks'], $row['matric_per'], $row['fsc'], $fsc_year, $row['fsc_inst'], $row['fsc_roll'], $row['fsc_marks'], $row['fsc_tmarks'], $row['fsc_per'], $row['mcat'], $mcat_year, $row['mcat_inst'], $row['mcat_roll'], $row['mcat_marks'], $row['mcat_tmarks'], $row['mcat_per']);
            $lineData = array( $row['name'], $row['fname'],$row['cnic'], $row['gender'], $row['dob'], $row['caddress'], $row['app_mob'],$row['email'], $matric_year,trimNumber($row['matric_tmarks']),  trimNumber($row['matric_marks']), ($row['matric_year'] == 0) ? "" : round((trimNumber($row['matric_marks'])/trimNumber($row['matric_tmarks']))*10,2), $fsc_year,  trimNumber($row['fsc_tmarks']),trimNumber($row['fsc_marks']), ($row['fsc_year'] == 0) ? "" :round((trimNumber($row['fsc_marks'])/trimNumber($row['fsc_tmarks']))*40,2), $mcat_year,$row['mcat_roll'],trimNumber($row['mcat_tmarks']), trimNumber($row['mcat_marks']), ($row['mcat_year'] == 0) ? "" : round((trimNumber($row['mcat_marks'])/trimNumber($row['mcat_tmarks']))*50,2),($row['matric_year'] != 0 && $row['fsc_year'] != 0 && $row['mcat_year'] != 0) ? trimNumber($row['t_aggregate']) : "0");
        ...
        <form action="" method="POST">
            ...
            <div style="margin-top:30px; margin-bottom:20px; float:right;">
                ...
                <a href="logout.php" style="text-decoration:none;">
                    <button type="button" class="btn btn-info" style="">LOGOUT</button>
                </a>
            </div>
...
<div class="container-fluid">
    <div class="row" style="padding-top:15px; padding-bottom:15px;">
        <div class="col-md-12">
            <h2 style="font-size:28px;text-align:center; font-family:Constantia, 'Lucida Bright', 'DejaVu Serif', Georgia, serif"><?php echo $course != "" ? "$headingText (".courseTitle($course).")" : $headingText; ?></h2>
        </div>
    </div>
        ...
        <table class="table table-bordered" style="text-align:center;">
            <thead>
                <tr style="background-color:#000; color:#FFF;">
                    ...
                    <th class="text-center">SSC Total Marks</th>
                    <th class="text-center">SSC Marks Obtained</th>
                    <th class="text-center">SSC Obtained Weightage (10%)</th>
                    <!-- <th class="text-center">F.Sc/A_Level</th> -->
                    <th class="text-center">FSC/A-level Passing year</th>
                    <!-- <th class="text-center">Institution's_Name</th> -->
                    <!-- <th class="text-center">Roll_No.</th> -->
                    <th class="text-center">HSSC/IBCC Equilvalence Total Marks</th>
                    <th class="text-center">HSSC/IBCC Equilvalence Marks Obtained</th>
                    <th class="text-center">FSC Obtained Weightage (40%)</th>
                    <!-- <th class="text-center">MDCAT</th> -->
                    <th class="text-center">MDCAT Year</th>
                    <!-- <th class="text-center">Institution's_Name</th> -->
                    <th class="text-center">MDCAT Roll No.</th>
                    <th class="text-center">MDCAT Total Marks</th>
                    <th class="text-center">MDCAT Marks Obtained</th>
                    <th class="text-center">MDCAT Obtained Weightage (50%)</th>
                    <!-- <th class="text-center"><img src="assets/sort-down-alt.svg" /> Aggregate</th> -->
                    <th class="text-center">
                        <form action="" method="POST">
                            <button type="submit" name="t_aggregate_order" class="btn btn-default btn-sm" value="<?php echo $t_aggregate_order == "DESC" ? "ASC" : "DESC"; ?>">
                                <span class="glyphicon <?php echo $t_aggregate_order == "DESC" ? "glyphicon-sort-by-attributes-alt" : "glyphicon-sort-by-attributes"; ?>"></span> Total Aggregate
                            </button>
                        </form>
                    </th>
                </tr>
            ...
            <tbody>
                ...
                while ($row = mysqli_fetch_array($q)) { ?>
                    <tr>
                        ...
                        <td><?php echo ($row['matric_year'] == 0) ? "": round((trimNumber($row['matric_marks'])/trimNumber($row['matric_tmarks']))*10,2); ?></td>
                        ...
                        <td><?php echo ($row['fsc_year'] == 0) ? "" : round((trimNumber($row['fsc_marks'])/trimNumber($row['fsc_tmarks']))*40,2); ?></td>
                        ...
                        <td><?php echo trimNumber($row['mcat_tmarks']); ?></td>
                        <td><?php echo $row['mcat_marks']; ?></td>
                        <td><?php echo ($row['mcat_year'] == 0) ? "":round((trimNumber($row['mcat_marks'])/trimNumber($row['mcat_tmarks']))*50,2); ?></td>
                        <td><?php echo ($row['matric_year'] != 0 && $row['fsc_year'] != 0 && $row['mcat_year'] != 0) ? 
                        $row['t_aggregate'] : "0"; ?></td>
                    </tr>
                <?php $count++;
                } }else{ ?>
